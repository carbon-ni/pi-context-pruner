...
describe("configureAutoPrune", () => {
  it("enables auto-prune with the default threshold", () => {
    expect(configureAutoPrune("")).toEqual({
      enabled: true,
      thresholdPercent: 60,
    });
  });
  ...
  it("disables auto-prune when threshold is zero", () => {
    expect(configureAutoPrune("0")).toEqual({
      enabled: false,
      thresholdPercent: 0,
    });
  });
...
describe("shouldAutoPrune", () => {
  it("does not prune when disabled", () => {
    expect(
      shouldAutoPrune({ percent: 90 }, { enabled: false, thresholdPercent: 0 }),
    ).toMatchObject({
      shouldPrune: false,
  ...
  it("prunes when context usage reaches threshold", () => {
    expect(
      shouldAutoPrune({ percent: 75 }, { enabled: true, thresholdPercent: 70 }),
    ).toMatchObject({
      shouldPrune: true,
      usagePercent: 75,
    });
  ...
  it("does not prune before threshold", () => {
    expect(
      shouldAutoPrune({ percent: 69 }, { enabled: true, thresholdPercent: 70 }),
    ).toMatchObject({
      shouldPrune: false,
      usagePercent: 69,
    });
  ...
  it("does not prune when usage is unavailable", () => {
    expect(
      shouldAutoPrune(undefined, { enabled: true, thresholdPercent: 70 }),
    ).toMatchObject({
      shouldPrune: false,
