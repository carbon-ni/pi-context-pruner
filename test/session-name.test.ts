...
import { prunedSessionName } from "../src/index.js";
import type { PruneStats } from "../src/types.js";

const stats = (sourceApproxTokens: number, keptApproxTokens: number): PruneStats => ({
	sourceMessages: 10,
	keptMessages: 5,
	droppedMessages: 5,
	sourceApproxTokens,
...
describe("prunedSessionName", () => {
	...
	it("increments prune-only sessions without adding suffixes", () => {
		expect(
			prunedSessionName("prune:reasoning · ×2 · -80% [prune:reasoning]", "tools", stats(0, 0)),
